...
void tex_unit::setfilter(GLuint minf, GLuint maxf)
{
    minfilter = minf;
    magfilter = maxf;
}
...
//1d
void tex_unit::create(int w, int b, GLvoid* p)
{
    m_width = w;
    ...
    glTexParameteri(T, GL_TEXTURE_MIN_FILTER, minfilter); //GL_LINEAR);	
    glTexParameteri(T, GL_TEXTURE_MAG_FILTER, magfilter); //GL_LINEAR);
    ...
    if( printOpenGLError() )
    {
        std::cout << __FILE__ << ":" <<  __func__ << "()," << m_texture_id << "\n";
        exit(1);
    }
//     else
//     {
//         std::cout << "texture created, size:" << w  << "\n";
//     }
    unbind();
}
...
//     else
//     {
//         std::cout << "texture created, size:" << w << "/" << h << "/" << d << "\n";
//     }
    unbind();
}
...
void tex_unit::create(int w, int h, int b, GLvoid* p[6])
{
    m_width = w;
    m_height = h;
    ...
    //tex gen mode: GL_NORMAL_MAP, GL_REFLECTION_MAP
...
//1d
void tex_unit::update_subimage(int ox, int w,  GLvoid* p)
{
    bind();
    glTexSubImage1D(T, 0, 
                    ox, 
...
void tex_unit::check_error_2d(int ox, int oy, int w, int h)
{
    assert(ox >= 0 && ox < m_width);
    assert(oy >= 0 && oy < m_height);

    int bx = ox + w;
    int by = oy + h;
    assert(bx > 0 && bx <= m_width);
    assert(by > 0 && by <= m_height);
}
