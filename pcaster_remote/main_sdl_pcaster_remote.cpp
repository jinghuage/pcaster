...
void idle()
{
    ...
    if(myrank==0) streamer->deliver_output();
...
//     window_h = pcaster_options::render_hei;
    ...
    if(myrank == 0)
    {
        tracker = new Event_Relayer(myrank, runsize, 
                                    ...
                                    GL_IO::MEM_COPY, 1);
#ifdef _STREAMING
        tracker->init_data_mover( Socket_Data_Mover::NET_CLIENT,
                              pcaster_options::viewer, 5900);
#endif
    }
    else
    {
    ...
    tracker->mapto_world_transform(objpos, objrot); 
...
//     }
    ...
    if (SDL_Init(SDL_INIT_VIDEO) == 0)
    {
        ...
        {            
            ...
            if(myrank==0) streamer->register_image_producer( pcaster );
